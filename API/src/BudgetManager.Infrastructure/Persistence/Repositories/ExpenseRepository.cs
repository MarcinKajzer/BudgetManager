...
namespace BudgetManager.Infrastructure.Persistence.Repositories
{
    public class ExpenseRepository : IExpenseRepository
    {
        ...
        {
            var subcategory = InMemoryStorage.categories.FirstOrDefault(c => c.Subcategories.Any(s => s.Id == expense.SubcategoryId))?.Subcategories.First(sc => sc.Id == expense.SubcategoryId);
            subcategory.Expenses.Add(expense);
        }
        public Expense? Get(Guid id)
        {
            var subcategory = InMemoryStorage.categories.FirstOrDefault(c => c.Subcategories.Any(s => s.Expenses.Any(e => e.Id == id)))?.Subcategories.First(sc => sc.Expenses.Any(e => e.Id == id));
