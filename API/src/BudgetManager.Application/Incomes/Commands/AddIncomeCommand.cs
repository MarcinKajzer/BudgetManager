...
{
    public record AddIncomeCommand(Guid SubcategoryId, DateTime Date, decimal Amount, string Comment) : IRequest<Guid>;
    ...
    public class AddIncomeHandler : IRequestHandler<AddIncomeCommand, Guid>
    {
        ...
        {
            var category = _categoryRepository.Get(request.SubcategoryId) ?? throw new NotFoundException();
